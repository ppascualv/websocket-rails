...
class @WebSocketRails
  ...
  subscribe: (channel_name, success_callback, failure_callback, params) =>
    unless @channels[channel_name]?
      channel = new WebSocketRails.Channel channel_name, params, @, false, success_callback, failure_callback
      @channels[channel_name] = channel
      channel
    else
      @channels[channel_name]

  subscribe_private: (channel_name, params, success_callback, failure_callback) =>
    unless @channels[channel_name]?
      channel = new WebSocketRails.Channel [channel_name, params], @, true, success_callback, failure_callback
      @channels[channel_name] = channel
      channel
    else
      @channels[channel_name]
